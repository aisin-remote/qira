<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('PICA Quality - Internal Problem') }}
        </h2>
    </x-slot>
    ...
    <div class="p-6 overflow-hidden bg-white rounded-md shadow-md dark:bg-dark-eval-1 relative">
        ...
        <div class="overflow-x-auto mt-4 shadow-md sm:rounded-lg">
            <table id="myTableCost" class="w-full text-xs md:text-xs text-left text-gray-500 border border-gray-300 table-sort">
                ...
                <tbody>
                    @if (count($internalIssues) > 0)
                        @foreach($internalIssues as $data)
                            <tr>
                            ...
                            <td class="px-4 py-2">
                                ...
                                <td class="px-4 py-2">
                                    <a href="{{ route('download.excel.internal', $data->id) }}"class="text-blue-500 underline" download>
                                ...
                                <td class="px-4 py-2 text-center">
                                    <a href="{{ route('pica.internal.edit', $data->id) }}" class="text-blue-500 hover:text-blue-700 font-bold">Edit</a>
                                    |
                                    <form action="{{ route('internal.delete', ['id' => $data->id]) }}" method="POST" style="display: inline;">
                                        @csrf
                                        @method('DELETE')
